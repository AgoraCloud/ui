...
import { BaseAdminPermissionsModel, AdminUserPermissionsModel, WorkspaceAdminPermissionsModel } from "app/res/Auth";
...
export class BaseAdminPermissionsFormModel extends FormModel<permissions_i>{
    ...
    onSelectPermission(permission: string){
        return ()=>{
            let change; 
            if(this.hasPermission(permission)) change = _.without(this.data.permissions, permission)
...
export class WorkspacePermissionsFormModel extends BaseAdminPermissionsFormModel{
    constructor(public permissions: WorkspaceAdminPermissionsModel, public workspace: WorkspaceModel){
        super(permissions)
