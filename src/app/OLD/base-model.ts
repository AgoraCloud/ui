...
export class BaseModelCollection<
  C extends BaseModelItem<any>,
> extends BaseModel<any> {
  /**
  ...
  public async load(url: string) {
    ...
    if (this.response.status >= 200 && this.response.status < 300) {
      if (this.iteratorKey)
        this.collection = _.get(this.responseData, this.iteratorKey).map(
          (data) => new this.ItemModel(this, data),
        );
      else
        this.collection = this.responseData.map(
          (data) => new this.ItemModel(this, data),
        );
      this.state = 'loaded';
    } else {
...
export class BaseModelCollectionAsync<
  C extends BaseModelItemAsync<any>,
> extends BaseModelCollection<C> {
...
export class BaseModelItemAsync<
  T extends { id: string },
> extends BaseModelItem<T> {
  load = async () => {
